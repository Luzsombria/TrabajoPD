...
libres c = [p | p <- lista, valido p c]
	where lista = [(1,1),(1,2),(1,3),(2,1),(2,2),(2,3),(3,1),(3,2),(3,3)]

-- Función de creacion de arbol
creaArbol :: Cuadricula -> Arbol Cuadricula
creaArbol c | llena c = (N c [])
            | otherwise = (N c (map creaArbol (movimientos c)))

-- Función de jugadas futuras segun su disponibilidad
movimientos :: Cuadricula -> [Cuadricula]
movimientos c | hay3EnRaya c = []
              | otherwise = map (actualizaValor pos c) (libres c)
					where pos = head (libres c)
...
mejorMov = seleccion . maximiza . poda profBus . construyeArbol
...
minimiza :: Arbol Cuadricula -> Arbol (Valor, Cuadricula)
...
poda :: Int -> Arbol -> Arbol a
poda prof (N a as) | prof == 0 = N x [] | otherwise = N x (map (poda (prof-1)) as)
